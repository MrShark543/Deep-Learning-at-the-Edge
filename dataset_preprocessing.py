...
class DatasetPreprocessor:
    """Preprocess ShanghaiTech Part B dataset"""
    ...
    def create_directory_structure(self):
        ...
        for split in ['train', 'test']:
            for folder in ['images', 'ground_truth', 'density_maps']:
                dir_path = self.target_root / 'part_B' / split / folder
                dir_path.mkdir(parents=True, exist_ok=True)
    ...
    def process_single_image(self, img_path, gt_path, target_img_path,
                             target_gt_path, target_den_path):
    ...
    def process_dataset(self):
        ...
        for split in ['train', 'test']:
            ...
            source_img_dir = self.source_root / "part_B_final" / f"{split}_data" / "images"
            source_gt_dir = self.source_root / "part_B_final" / f"{split}_data" / "ground_truth"

            target_img_dir = self.target_root / "part_B" / split / "images"
            target_gt_dir = self.target_root / "part_B" / split / "ground_truth"
            target_den_dir = self.target_root / "part_B" / split / "density_maps"
            ...
            for img_path in tqdm(img_files, desc=f"Processing Part B {split}"):
                gt_name = "GT_" + img_path.name.replace('.jpg', '.mat')
                gt_path = source_gt_dir / gt_name

                if not gt_path.exists():
                    stats['part_B']['errors'].append(img_path.name)
                    continue
                ...
                if success:
                    stats['part_B'][split] += 1

            print(f"  Processed: {stats['part_B'][split]} images")
    ...
    def print_summary(self, stats):
        ...
        train_count = stats['part_B']['train']
        test_count = stats['part_B']['test']
        error_count = len(stats['part_B'].get('errors', []))
        ...
        print(f"\nDataset saved to: {self.target_root}")
    ...
    def verify_dataset(self, num_samples=3):
        ...
        img_dir = self.target_root / "part_B" / "train" / "images"
        gt_dir = self.target_root / "part_B" / "train" / "ground_truth"
        den_dir = self.target_root / "part_B" / "train" / "density_maps"
        ...
        for img_path in img_files:
            ...
            print(f"\n{img_path.name}:")
            print(f"  Image shape: {image.shape} (should be 256x256x3)")
            print(f"  Point count: {gt_data['count']}")
            print(f"  Density shape: {density_map.shape} (should be 32x32)")
            print(f"  Density sum: {np.sum(density_map):.2f}")
            print(f"  Count vs sum diff: {abs(gt_data['count'] - np.sum(density_map)):.4f}")

            assert image.shape == (256, 256, 3), "Image shape mismatch!"
            assert density_map.shape == (32, 32), "Density shape mismatch!"
            assert abs(np.sum(density_map) - gt_data['count']) < 0.1, "Density sum doesn't match count!"
...
def verify_no_data_leakage():
    ...
    print("\nVerifying Part B data split:")

    train_dir = target_root / "part_B" / "train" / "images"
    test_dir = target_root / "part_B" / "test" / "images"

    if not train_dir.exists() or not test_dir.exists():
        print("  Directories not found for Part B")
        return
...
def preprocess_shanghaitech_dataset_256():
    """Main function to preprocess Part B dataset at 256x256"""
    ...
    print("\nDataset preprocessing complete!")
    print("256x256 RGB dataset ready!")
    print("Density maps are 32x32 (1/8 scale)")
